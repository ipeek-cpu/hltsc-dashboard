...
const oauthServers: Map<number, http.Server> = new Map();
...
function setupIPC(): void {
  // Preview controls
  ipcMain.handle('preview:load', async (_, url: string) => {
    if (previewManager) {
  ...
  ipcMain.handle('preview:set-bounds', async (_, bounds: { x: number; y: number; width: number; height: number }) => {
    if (previewManager) {
  ...
  ipcMain.handle('app:get-path', (_, name: string) => {
    return app.getPath(name as 'home' | 'appData' | 'userData' | 'temp' | 'exe' | 'desktop' | 'documents' | 'downloads');
  });
  ...
  ipcMain.handle('oauth:start-server', async (_, requestedPort?: number) => {
  ...
  ipcMain.handle('oauth:stop-server', async (_, port: number) => {
